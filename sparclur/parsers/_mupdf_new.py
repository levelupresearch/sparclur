...
class NuPDF(Tracer, Hybrid, Reforger):
    """MuPDF parser"""
    def __init__(self, doc: Union[str, bytes],
        ...
        skip_check = _get_config_param(NuPDF, config, 'skip_check', skip_check, False)
        hash_exclude = _get_config_param(NuPDF, config, 'hash_exclude', hash_exclude, None)
        parse_streams = _get_config_param(NuPDF, config, 'parse_streams', parse_streams, True)
        binary_path = _get_config_param(NuPDF, config, 'binary_path', binary_path, None)
        temp_folders_dir = _get_config_param(NuPDF, config, 'temp_folders_dir', temp_folders_dir, None)
        dpi = _get_config_param(NuPDF, config, 'dpi', dpi, 200)
        cache_renders = _get_config_param(NuPDF, config, 'cache_renders', cache_renders, False)
        timeout = _get_config_param(NuPDF, config, 'timeout', timeout, None)
        ocr = _get_config_param(NuPDF, config, 'ocr', ocr, False)
...
#         self._cmd_path = 'mutool clean' if binary_path is None else binary_path
    ...
    def _reforge(self):
        ...
        try:
            ...
            warnings = fitz.TOOLS.mupdf_warnings()
            error_arr = [message for message in warnings.split('\n') if len(message) > 0]
            self._reforged = raw
            self._successfully_reforged = True
            self._reforge_result = 'Successfully reforged'
        except FunctionTimedOut:
            warnings = fitz.TOOLS.mupdf_warnings()
            error_arr = [message for message in warnings.split('\n') if len(message) > 0]
            error_arr.insert(0, 'Error: Parsing timed out: %i' % (self._timeout or 600))
            self._successfully_reforged = False
            self._reforge_result = '[' + ', '.join(error_arr) + ']'
        except Exception as e:
            warnings = fitz.TOOLS.mupdf_warnings()
            error_arr = [message for message in warnings.split('\n') if len(message) > 0]
            error_arr.insert(0, 'Error: %s' % str(e))
            self._successfully_reforged = False
            self._reforge_result = '[' + ', '.join(error_arr) + ']'
        self._messages = ['No warnings'] if len(error_arr) == 0 else error_arr
    ...
    def _mutool_clean(self):
        if isinstance(self._doc, bytes):
            stream = BytesIO(self._doc)
            filename = None
        else:
            filename = self._doc
            stream = None
        with fitz.open(filename=filename, stream=stream, filetype="pdf") as doc:
            cleaned = doc.tobytes(clean=self._parse_streams)
        return cleaned

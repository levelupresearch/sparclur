...
def _worker(entry):
    ...
    for (name, parser) in renderers.items():
        try:
            ...
            orig: Renderer = parser(doc_path=orig_file, dpi=dpi, timout=timeout, cache_render=True, **args)
            mod: Renderer = parser(doc_path=mod_file, dpi=dpi, timout=timeout, cache_render=True, **args)
